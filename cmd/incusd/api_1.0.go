...
var api10 = []APIEndpoint{
	...
	instanceStateCmd,
	instanceXlabRootReleaseCmd,
	instanceXlabRootInspectionCmd,
	instanceXlabRootAdoptCmd,
	instanceXlabRootForgetCmd,
	instanceXlabRootPrepareCmd,
	instanceAccessCmd,
	instanceDebugMemoryCmd,
	instanceDebugRepairCmd,
	eventsCmd,
	imageAliasCmd,
	imageAliasesCmd,
	imageCmd,
...
func doApi10Update(d *Daemon, r *http.Request, req api.ServerPut, patch bool) response.Response {
	...
	err := s.DB.Node.Transaction(r.Context(), func(ctx context.Context, tx *db.NodeTx) error {
		...
		if nodeValues["storage.logs_volume"] != "" && nodeValues["storage.logs_volume"] != newNodeConfig.StorageLogsVolume() {
			err := daemonStorageValidate(s, "logs", nodeValues["storage.logs_volume"])
			if err != nil {
